<template>
    <div class="flex flex-col items-stretch justify-start w-full overflow-y-hidden bg-white border-l-2 border-r-2 border-gray-100 lg:rounded-r-md xl:rounded-none">
        ...
        <form enctype="multipart/form-data" v-show="currentMiddleSection === 'topic-conversations'" @submit.prevent="writeMessage" class="flex flex-row items-center justify-between p-3 border-t-2 border-gray-200">
            <div class="">
...
export default {
    ...
    methods: {
        ...
        onFilePicked (e) {
            e.preventDefault();
            let currentObj = e.target.files[0];
            let topicId = this.$page.url.split('=')[1];
            const config = {
                    headers: {
            ...
            formData.append('file', currentObj);
            formData.append('topicId', topicId);

            axios.post('/topic/file', formData, { config }).then((res) => {
                console.log(res);
            });
