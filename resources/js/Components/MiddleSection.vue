<template>
    <div class="flex flex-col items-stretch justify-start w-full overflow-y-hidden bg-white border-l-2 border-r-2 border-gray-100 lg:rounded-r-md xl:rounded-none">
        ...
        <div v-if="$page.props.user.current_team" class="flex flex-row items-center justify-between px-3 py-2 border-b-2 border-gray-100 bg-gray-50 bg-opacity-40">
            <div class="flex items-center">
                ...
                <h2>
                    ...
                    <inertia-link v-if="middleSection === 'topics'" :href="route('home')" :only="['middleSection']">
                    ...
                    <inertia-link v-if="middleSection === 'topic-conversations'" :href="route('home')" :only="['middleSection']">
                        <span class="font-semibold text-blue-400">
                            <span class="text-gray-500"> / </span>
                            <span>Topics</span>
                            <span class="text-gray-500"> / </span>
        ...
        <form  v-show="currentMiddleSection === 'topic-conversations'" @submit.prevent="writeMessage" class="flex flex-row items-center justify-between p-3 border-t-2 border-gray-200">
            <div class="">
                <button @click="onPickFile" type="button" class="p-2 text-gray-400 rounded-full hover:text-gray-600 hover:bg-gray-100 focus:outline-none focus:ring" aria-label="Upload a files">
                    <svg class="w-6 h-6 fill-current" viewBox="0 0 20 20">
            ...
            <div class="flex-1 px-3">
                <input
                    ...
                    class="w-full py-2 pl-2 mr-2 text-sm text-gray-700 placeholder-gray-500 bg-gray-600 border-gray-300 rounded-md outline-none search-input bg-opacity-10 focus:border-gray-500 focus:ring-transparent focus:outline-none"
                />
            </div>
            <div class="flex flex-row">
...
export default {
    ...
    methods: {
        ...
        writeMessage() {
            this.form.post(route('conversation.store', { topicId: this.$page.url.charAt(this.$page.url.search('topicId') + 8) }), {
                forceFormData: true,
                preserveScroll: true,
                onSuccess: () => {
                    this.form.reset();
