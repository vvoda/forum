                ...
                <div class="w-full p-1">
                    <div class="flex items-center w-full p-1">
                        ...
                        <ul v-if="$page.props.user.all_teams" class="overflow-y-auto groupList">
                            <li
                                @click.prevent="switchToTeam({ teamId: team.id, isFriend: !!team.user_id && !!team.friend_id })"
                                v-for="team in getCombinedGroups()"
                                :key="team.id"
                            ...
                            >
                                <img :src="team.user_id && team.friend_id ? '/user.png' : '/group.png'" class="w-12 h-12 mr-4 bg-gray-700 rounded-full contrast" alt="">
                                <div class="flex flex-col justify-center w-full">
                                    <div class="flex flex-row items-center justify-between">
                                        <h2 class="text-xs font-bold" v-if="!team.friend && !team.user">{{ team['name']}}</h2>
                                        <div v-else>
                                            <h2 class="text-xs font-bold" v-if="$page.props.user.id === team.friend_id">{{ team.user.name}}</h2>
                                            <h2 class="text-xs font-bold" v-if="$page.props.user.id === team.user_id">{{ team.friend['name'] }}</h2>
                                        </div>
...
export default {
    ...
    methods: {
        getTeams() {
            let list = [];
            for (const [key, value] of Object.entries(this.$page.props.user.all_teams)) {
                list.push(value);
            }
            return list;
        },
        getCombinedGroups() {
            return [...this.getTeams(), ...this.contacts]
        }
    }
}
</script>
